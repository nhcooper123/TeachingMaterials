...
We can also plot with slightly nicer colour-blind friendly colours from the package `viridisLite` (have a look at the options and choose your favourite). 
...
BAMMtools comes with a couple of inbuilt datasets on whales, primates and fishes. Choose one of these and see if you can recreate the analyses above. how many rate shifts can you see? How many macroevolutionary regimes? 

To start you off here is the first bit of code you'll need for each of the examples (starting at the point we make `edata` above).

```{r}
# Whales
data(whales, events.whales)
whale.edata <- getEventData(whales, events.whales, burnin = 0.25, nsamples = 500,
...
primate.edata <- getEventData(primates, events.primates, burnin = 0.25, nsamples = 500,
                   type = 'trait')
```
...
fish.edata <- getEventData(fishes, events.fishes, burnin = 0.25, nsamples = 500,
                   type = 'trait')
```

1. What is different about the fishes and primates examples, compared to the whales and marsupials examples? Use the helpfiles to find out.
2. For primates, what's the probability of 19 shifts?
3. For whales, what is the mean speciation rate?
4. For fishes, how many distinct shift configurations are there in the credible shift set, where expected shifts = 1, threshold = 5, and set.limit = 0.95?
