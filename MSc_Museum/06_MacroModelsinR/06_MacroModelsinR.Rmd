...
The aims of this practical are to learn how to use R to fit macroevolutionary models in R to continuous and discrete traits.
...
# Read in tree
magicaltree <- read.nexus("magical.nex") 
...
## 3.1 Models of continuous trait evolution
...
a is the strength of evolutionary force that returns traits back towards the long-term mean if they evolve away from it. 
a is sometimes referred to as the “rubber band” parameter because of the way it forces traits back towards m.
...
In a Brownian motion (BM) model we estimate the Brownian rate parameter, s2 or `sigsq` in the output above, which is `1192.2` and the value of the trait at the root of the tree, X0 or `z0` in the output above, which is `5.415`.
...
As `alpha = 2.71` here, it suggests that there is evolution towards a body mass optimum.
...
```
FIXXXXXX
For these data, the rate half-life is ~ 18 million years, which means over the course of magical
creature evolution, body size rate has undergone ~ 4 rate halvings. 

## 3.3 Comparing models of evolution
...
There is much debate about how big of a difference in AIC values can be classed as substantial improvement to a model fit (it ranges from 2-10 AIC units). 
...
Alternatively we can use DAIC or AIC weights to compare our models using the following code and the geiger function aicw:
...
Using DAIC we can conclude that the OU model is the best fit to the data.

## 3.4 Problems with convergence and bounds
Above we have mentioned the default bounds on each parameter. Sometimes these need to be changed because the model will not converge. 
This happens when the likelihood surface has long flat ridges that cause the likelihood search to get “stuck” (this is particularly common under the
OU model). 
...
## 4. Fitting models of evolution to discrete data (regime dependent evolution) 
...
# Change the margins so the tree fits
par(mar = c(0,0,0,0)) 
...
legend("bottomleft",legend=c("solitary", "social"), 
       pch = 15, cex=1.3, col=c("lightblue", "plum"), bty = "n")
# Put the margins back to the default
par(mar = c(5,4,4,2)) 
```
...
## 4.2 Fitting the models using fitDiscret
Let's try these models with our magical dataset using `fitDiscrete`.
...
The all-rates-different model is less strongly supported (AICcW = 0.38) than the equal rates model (AICcW = 0.61). 
...
```{r}
asr <- ace(mydata[,"SocialStatus"], mytree, type="discrete", model = "ARD")
```
...
Now it's obvious that our intuition was correct – we reconstruct solitary as the ancestral state for magicals, but with multiple transitions to sociality. 
...
Specifically, the rate for regime 1 appears to be twice that of regime 2. So rates of body size evolution seem to be faster for solitary magicals than for social magicals. 
...
We infer an optimal size for solitary magicals of 5.99 ln(mass) and an optimal mass of 8.6 ln(mass) for social magicals. 
...
* Arnold, C., L. J. Matthews, and C. L. Nunn. 2010. The 10ktrees website: a new online resource for magical phylogeny. Evolutionary Anthropology: Issues, News, and Reviews 19:114–118.
