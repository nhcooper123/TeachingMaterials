...
r(t) = \sigma^2e(at)
\end{equation}
Where $r(t)$ is the rate of evolution at time $t$, $\sigma^2$ is the initial value of the Brownian rate parameter, i.e. the initial rate of evolution, $a$ is the rate change parameter, and $t$ is time. The value of $a$ is generally less than or equal to 0 (note that you can force a to be greater than zero by changing the bounds (see \subsection{Problems with convergence and bounds} , however, this will only . When $a$ is negative, rates of evolution decrease through time.

a > 0 is non-identifiable for extant only data (see Slater et al. 2012)
